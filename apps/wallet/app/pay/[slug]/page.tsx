...
export default function PayPage() {
  ...
  async function triggerPayment() {
    ...
    if (balance < 0.01) {
      setState("needsFunds");
      setMessage("Your balance is too low. Please add funds first.");
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setState("error");
        setMessage(`Error ${response.status} — please try again.`);
      }
    } catch (e: any) {
      setState("error");
  ...
  const UI: Record<State, React.ReactNode> = {
    ...
    success: (
      <div style={{ textAlign: "center" }}>
        ...
        <p style={{ fontSize: 13, color: "var(--text-2)", marginTop: 6 }}>{message || "Content unlocked."}</p>
      </div>
    ),
    error: (
      <div style={{ textAlign: "center" }}>
    ...
    needsFunds: (
      <div style={{ textAlign: "center" }}>
        <p style={{ fontSize: 16, fontWeight: 500, marginBottom: 8 }}>Insufficient balance</p>
        <p style={{ fontSize: 13, color: "var(--text-2)", marginBottom: 20 }}>{message}</p>
        <a href="https://wallet.trnpk.net"><button>Add funds</button></a>
      </div>
    ),
  };
